...
class Movie {
    ...
    setModal = () => {
        const masterDiv = document.getElementById('master-container')
        masterDiv.innerHTML = this._modalHtml

        getSimilar( this.id ).then( similarArray => this.similarArray = similarArray )

        this.lastSimilar += 3
        const recArray = this.similarArray.slice(0,this.lastSimilar).map( ( { id, title, backdrop_path } ) => {
            if ( backdrop_path ) {
                return `<img class="similar-sqr" data-movieid="${id}" src="https://image.tmdb.org/t/p/w1280/${backdrop_path}" alt="poster for ${ title }" />`
            }
        } ).join('')
        ...
        <div class="similar--movies">
            ${recArray}
            <button data-movieid="${ this.id }" id="load-more-similar" class="invisible-btn img-link-btn similar-sqr">
                <img data-movieid="${ this.id }" src="./img/vectors/add.png" alt="more movies" />
            </button>
        </div>`

        const similarContainer = document.getElementById('similar-container')
        similarContainer.innerHTML = recommendedDiv

        const addRecsBtn = document.getElementById('load-more-similar')
        addRecsBtn.addEventListener('click', () => {
            this.setModal()
        } )
    }
    getMovieDiv = () => {
        // for most watched container
        return (`
    ...
    // getRecommendedMoviesDiv = () => {
    //     this.lastRecommended += 3
    //     const imgArray = this.recommendedArray.slice(0, this.lastRecommended).map( ( { backdrop_path, title, id } ) => {
    //         if ( backdrop_path ) {
    //             return `<img class="similar-sqr" data-movieid="${id}" src=https://image.tmdb.org/t/p/w1280/${backdrop_path} alt='poster for ${ title }' />`
    //         }
    //     } )
...
function getModalAndSetUp( movieId ){
    getMovieById( movieId )
        .then( movieData => {
            const { id, title, genres, vote_average, overview, backdrop_path, release_date, original_language } = movieData

            const movieObj = new Movie ( id, title, genres, vote_average, overview, backdrop_path, release_date, original_language )

            console.log( id, title, genres, vote_average, overview, backdrop_path, release_date, original_language )

            movieObj.setModal()            
        } )
}

// function addMoreMovies( movieObj ) {
//     const recommendedArray = movieObj.getRecommendedMoviesDiv()
//     const recommendedDiv = `
//     <p class="similar--title text--bold">Similar Movies:</p>
//     <div class="similar--movies">
//         ${recommendedArray}
//         <button data-movieid="${ movieObj.id }" id="load-more-similar" class="invisible-btn img-link-btn similar-sqr">
//             <img data-movieid="${ movieObj.id }" src="./img/vectors/add.png" alt="more movies" />
//         </button>
//     </div>`
//     document.getElementById('similar-container').innerHTML = recommendedDiv

//     document.getElementById('load-more-similar').addEventListener('click', () => {
//         console.log("load more")
//         document.getElementById('similar-results').innerHTML = movieObj.getRecommendedMoviesDiv()
//     } )
// }
...
function getDetailsHtml( movieId ) {
    getMovieById( movieId )
        .then( movieRes => {
            ...
            const movieModal = new Movie ( id, title, genres, vote_average, overview, backdrop_path, release_date, original_language )
            ...
            getRecommendedMovies( movieId )
                .then( moviesRec => {
                    ...
                    const recDiv = movieModal.getRecommendedMoviesDiv( moviesRec, movieModal.lastRecommended + 3)
                    document.getElementById('similar-container').innerHTML = recDiv
                    setUpRecommendedMovies()
